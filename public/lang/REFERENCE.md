...
Create FHIR resources from tabular data and submit them to a server. The table columns map to FHIR element paths (e.g. `code.coding.code`). After submission, you can assert the HTTP response status and inspect the OperationOutcome.
...
Given Client creates an allergy resource with:
  | resourceType       | code.coding.code | code.coding.display | code.text           |
...
| `the validation should pass` | Assert zero validation errors |
...
**Example — generate a Belgian Patient with specific values:**
```gherkin
Given generate test data from profile "http://hl7.org/fhir/be/StructureDefinition/be-patient" with:
  | name.given | name.family | birthDate  | gender |
  | Jan        | Pansen      | 1990-05-15 | male   |
```

**Example — define reusable mappings and data, then generate:**
```gherkin
Given define mappings "patientMap":
  | fhirPath    | element   |
...
And define data "patients":
  ...
  | Jan       | Pansen   |
  | Marie     | Dubois   |
When generate test data from profile "http://hl7.org/fhir/StructureDefinition/Patient" with mappings "patientMap" and data "patients" as "testPatient"
```
